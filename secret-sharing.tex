...
\begin{document}
	\pagenumbering{Alph}
	\begin{titlepage} % TODO: Definitionen kennzeichen :)
		\maketitle
		\thispagestyle{empty}
		\tableofcontents
		\thispagestyle{empty}
	...
	\begin{bew}
		Wir beweisen dies mithilfe vollständiger Induktion. Den trivialen Fall für $ \det V_1 = \det \begin{pmatrix}1\end{pmatrix} = 1 $ vernachlässigen wir hierbei.
		
		\begin{itemize}
			\item {\underline{\textbf{Induktionsanfang $ n=2 $:}}
				
				Der Induktionsanfang für $ n=2 $ ist schnell gezeigt:
				$$ \det (V_2) = \det \begin{pmatrix}1&a_1\\1&a_2\end{pmatrix} = 1 \cdot a_2 - 1 \cdot a_1 = \prod_{j=1, i=2} (a_i-a_j) = \prod_{1 \leq j < i \leq 2} (a_i-a_j).$$
			}
